   ...
   const tmpFileOut = path.join(tmpdir(), `${Crypto.randomBytes(6).readUIntLE(0, 6).toString(36)}.webp`)
    ...
    await new Promise((resolve, reject) => {
        ff(tmpFileIn)
            ...
            //    "libwebp",
            //    "-vf",
                "scale='min(512,iw)':min'(512,ih)',fps=30, pad=512:512:-1:-1:color=white@0.0, split [a][b]; [a] palettegen=reserve_transparent=off:transparency_color=ffffff [p]; [b][p] paletteuse"
            //])
            //.toFormat("webp")
            //.save(tmpFileOut)
    })
